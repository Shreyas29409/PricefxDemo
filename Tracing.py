    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            with allure.step("Select customer CH-0001"):
                ...
                # ✅ Fix strict mode: pick the first matching row
                row = page.get_by_role("row", name="Press SHIFT+SPACE to select").first
                row.wait_for(state="visible")
                row.get_by_label("", exact=True).click()
            ...
            with allure.step("Open workflow tab and approve quote"):
                ...
                page.locator(
                    ".ucButton.ucButton--hasIcon.ucButton--iconOnly.ucButton--iconOnlyColor-green > .ucButton__inner"
            ...
            # ✅ STOP tracing and save file (even if test fails)
            context.tracing.stop(path="trace.zip")
            browser.close()

      #to run the tracing use pytest Code\test_work.py -s -v --headed --tracing=on 
      # to view playwright show-trace
  #trace.zip is currently in the Reports folder
